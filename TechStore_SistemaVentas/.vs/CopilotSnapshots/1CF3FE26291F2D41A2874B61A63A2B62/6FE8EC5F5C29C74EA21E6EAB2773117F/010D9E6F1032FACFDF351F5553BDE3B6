...
namespace TechStore.Presentacion
{
    public partial class FormNuevaVenta : Form
    {
        ...
        private void FormNuevaVenta_Load(object sender, EventArgs e)
        {
            ...
            // Bind carrito once to avoid resetting DataSource which can cause CurrencyManager index issues
        ...
        private void ConfigurarDataGridView()
        {
            ...
            {
                DataPropertyName = "ProductoCodigo",
                HeaderText = "Código",
                Width = 100
            });
        ...
        private void CargarMetodosPago()
        {
            try
            {
                ...
                // ensure totals update when method changes
        ...
        {
            // BindingList notifies the grid automatically; just refresh display
            dgvCarrito.Refresh();
        }
        ...
        private void CalcularTotales()
        {
            ...
            // Determine client type discounts/surcharges
            decimal clientDiscountPercent = 0m; // applied only if Mayorista
            bool isMinorista = false;

            if (cmbCliente.SelectedIndex >= 0)
            {
                var cliente = (Cliente)cmbCliente.SelectedItem;
                if (cliente?.TipoCliente != null && cliente.TipoCliente.Nombre != null)
                {
                    var tipoName = cliente.TipoCliente.Nombre.Trim();
                    if (tipoName.Equals("Mayorista", StringComparison.InvariantCultureIgnoreCase))
                    {
                        clientDiscountPercent = cliente.TipoCliente.PorcentajeDescuento;
                    }
                    else if (tipoName.Equals("Minorista", StringComparison.InvariantCultureIgnoreCase))
                    {
                        isMinorista = true;
                    }
                }
            ...
            // Base total after client-type discount (if any)
            decimal clientDiscountAmount = subtotal * (clientDiscountPercent / 100m);
            decimal baseTotal = subtotal - clientDiscountAmount;

            // Payment-method discount (always evaluated)
            decimal paymentMethodDiscountPercent = 0m;
            if (cmbMetodoPago.SelectedIndex >= 0)
            {
                var metodo = (MetodoPago)cmbMetodoPago.SelectedItem;
                if (metodo != null && metodo.Nombre != null)
                {
                    var name = metodo.Nombre.ToLowerInvariant();
                    if (name.Contains("deb") || name.Contains("efectiv") || name.Contains("transfer"))
                    {
                        paymentMethodDiscountPercent = 15m;
                    }
                }
            }

            decimal paymentMethodDiscountAmount = baseTotal * (paymentMethodDiscountPercent / 100m);

            // Minorista surcharge: 10% of baseTotal (explicit requirement)
            decimal minoristaSurchargePercent = isMinorista ? 10m : 0m;
            decimal minoristaSurchargeAmount = baseTotal * (minoristaSurchargePercent / 100m);

            // Final totals
            decimal descuento = clientDiscountAmount + paymentMethodDiscountAmount; // show discounts applied
            decimal total = baseTotal - paymentMethodDiscountAmount + minoristaSurchargeAmount;
        ...
        private void btnRegistrarVenta_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string mensaje;
                bool resultado = _ventaNegocio.RegistrarVenta(venta, _carrito.ToList(), out mensaje);
